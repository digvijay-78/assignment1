...
A software company wants to reward its high-performing employees with a 10% bonus. 
However, only employees earning ₹50,000 or more are eligible for
the bonus.
...
Note: Use filter() to identify eligible employees and map() to calculate the updated 
salaries. Both operations must use lambda expressions.
Input Format
...
s=[35000,50000,62000,45000,70000,55000]
res=list(filter(lambda x:x>=50000,s))
res=list(map(lambda x:x+x*10/100,res))
print(res)