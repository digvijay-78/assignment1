...
for i in range(row):
	rows=[]
	for j in range(col):
              rows.append(int(input(f"Enter the element of [r{i},c{j}]: ")))
	m.append(rows)
print(m)
c=[]
for i in range(row):
...
books=namedtuple("books",["id","title","auther","price"])
...
print("Details :")
for x in b:
   print(x.id ,x.title,x.auther,x.price)
   if high.price<x.price:
       high=x
   if low.price>x.price:
       low=x
   total+=x.price
print("Most expensive book:")
print(high.id,high.title,high.auther,high.price)
print("cheapest book:")
print(low.id,low.title,low.auther,low.price)
print("Averag book price :")
average=total/n
print(average)
